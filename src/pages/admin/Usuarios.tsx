...
import { Search, RefreshCcw, ShieldCheck, Ban, CheckCircle2, Clock, Users as UsersIcon } from "lucide-react"
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          {row.original.status === "bloqueado" ? (
            <Button variant="ghost" size="icon" className="h-8 w-8 text-success-600" title="Desbloquear">
              <CheckCircle2 className="h-4 w-4" />
            </Button>
          ) : (
            <Button variant="ghost" size="icon" className="h-8 w-8 text-danger-600" title="Bloquear">
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard label="Última Sincronização AD" value="06:00" suffix="hoje" icon={Clock} tone="info" index={3} />
      </div>
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between space-y-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Provisionamento automático via grupos do AD — sem cadastro manual</p>
          </div>
