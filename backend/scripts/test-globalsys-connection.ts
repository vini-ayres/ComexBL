...
console.log('Domínio:', env.globalsys.domain ?? '(não definido)');
console.log('Auth mode:', env.globalsys.authMode ?? '(auto)');
...
if (result.connected) {
  console.log(`Resultado: Conexão OK (${result.responseTimeMs}ms, auth=${result.authMode})`);
} else {
