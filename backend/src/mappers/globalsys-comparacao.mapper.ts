...
const MASTER_COLUMN_FALLBACKS: Record<string, readonly string[]> = {
  ...
  shipperName: ['ShipperName', 'NM_AGENTE_INTERNACIONAL'],
  serviceTerm: ['ServiceTerm', 'NM_TIPO_MODALIDADE'],
  freightTerm: ['FreightTerm', 'NM_TIPO_PAGAMENTO', 'CD_TIPO_PAGAMENTO'],
  loadingPortName: ['LoadingPortName', 'NM_PORTO_ORIGEM'],
  dischargePortName: ['DischargePortName', 'NM_PORTO_DESTINO'],
...
export function mapGlobalSysMasterRecord(
  ...
  return {
    ...
    serviceTerm: mapped.serviceTerm ?? null,
...
export function mapGlobalSysHouseRecord(
  ...
  return {
    ...
    dischargePortCode: readColumnValue(record, 'CD_PORTO_DESTINO'),
    dischargePortName: mapped.dischargePortName ?? null,
    deliveryPortName: readFirstColumnValue(record, [
      'DeliveryPortName',
      'NM_DESTINO_FINAL',
    ]),
